    ...
    <section id="contacto" className="bg-brandAmberLight py-14 sm:py-16">
      <div className="container-page grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center">
        <div className="reveal-on-scroll">
          <p className="text-[11px] font-black uppercase tracking-[0.18em] text-brandGreenDark">{copy.eyebrow}</p>
        ...
        <div className="reveal-on-scroll flex flex-col gap-3 sm:flex-row lg:flex-col">
          <a href={contact.whatsapp} className="btn-primary min-w-56"><MessageCircle size={18} aria-hidden="true" /> WhatsApp</a>
          <a href={contact.email} className="inline-flex min-h-12 min-w-56 items-center justify-center gap-2 rounded-md border border-brandInk/20 bg-white px-5 py-3 text-sm font-black text-brandInk transition hover:-translate-y-0.5 hover:border-brandGreen"><Mail size={18} aria-hidden="true" /> {copy.email}</a>
          <div className="flex justify-center gap-2 lg:justify-start">
            {siteConfig.socialLinks.map(({ label, href }) => {
              const Icon = socialIcons[label as keyof typeof socialIcons];
              return <a key={label} href={href} target="_blank" rel="noreferrer" aria-label={`AgroEscudo · ${label}`} className="inline-flex h-11 w-11 items-center justify-center rounded-md border border-brandInk/20 bg-white/70 text-brandGreenDark transition hover:bg-white"><Icon size={18} aria-hidden="true" /></a>;
            })}
          </div>
        </div>
      </div>
